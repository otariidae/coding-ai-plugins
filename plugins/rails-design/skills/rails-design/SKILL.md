...
指摘を省いてよいのは、Rails のバージョンがその書き方を許していないときだけ
...
Rails のバージョンで使えない書き方は勧めない（`params.expect` は Rails 8+ など）。

ここで示すのは一般的な Rails Way であって、その repo の事情を汲んだ最終判断ではない。
慣習に合わせるか直すかを決めるのは読む人で、こちらの仕事は選択肢を見える形で出すこと。
